...
class FuturesExchange():
  ...
  def getTransaction(self, accountId = None):
    params = {
      "act": "OrderRecord",
      "aid": accountId,
      "startTime":    "",
  ...
  def getProfit(self, days= 7, accountId= None):
    today       = date.today()
    days        = int(days)
    oneWeekAgo  = today - timedelta(days= days)

    startDate   = "%s-%s-%s"% (oneWeekAgo.year, oneWeekAgo.month, oneWeekAgo.day)
    endDate     = "%s-%s-%s"% (today.year, today.month, today.day)
  ...
  def doRequest(self, params, accountId = None):
